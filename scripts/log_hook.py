...
_SENSITIVE_KEY_RE = re.compile(
    r"(?i)(authorization|cookie|password|passwd|secret|api[_-]?key|access[_-]?token|refresh[_-]?token)"
)
_SECRET_VALUE_RE = re.compile(
    r"(?i)(?:ssmcp_|sk-|gh[pousr]_)[A-Za-z0-9_-]{12,}|Bearer\s+[^\s,;]+"
)
_EMAIL_RE = re.compile(r"(?i)\b[A-Z0-9._%+-]+@[A-Z0-9.-]+\.[A-Z]{2,}\b")
_PHONE_RE = re.compile(r"(?<!\d)(?:\+84|0)[ .-]?[3-9](?:[ .-]?\d){8}(?!\d)")
_LOG_PAYLOAD_KEYS = {
    "prompt",
    "tool_input",
    "tool_response",
...
def sanitize_for_log(value):
    """Redact credentials and customer PII from untrusted hook payloads."""
    if isinstance(value, dict):
        return {
            key: "[REDACTED]" if _SENSITIVE_KEY_RE.search(str(key)) else sanitize_for_log(item)
...
def normalize(data: dict, tool: str) -> dict | None:
    ...
    return sanitize_log_entry(base)
